...
_Page({
  ...
  data: {
    ...
    pickerConfig: {
      ...
      limitStartTime: time.format_hour(
        new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 3)
      ),
      limitEndTime: time.format_hour(
    ...
    startTime:
      time.format_hour3(new Date(new Date().getTime())).toString() + ":00",
  ...
  onLoad: function(options) {
    ...
    that.getBannerList(); //获取优惠券

    that.getYHQ();
    console.log(that.data.card_type);
    console.log(that.data.cardIndex);
    console.log(options); // console.log(app.globalData.fixChannel)
  ...
  getDetectionType: function() {
    var that = this; // console.log(app.globalData.channel_id)

    var channel_id = that.data.channel.channel_id;
    var data = {
      flag: 0,
      //1 动态加载  0 全部加载 本来是参数dynamicTimeFlag
      channel_id: channel_id
    };
        ...
        if (res.success) {
          ...
          if (that.data.intoByScanFlag == true) {
            for (var i = 0; i < detectionTypeArr.length; i++) {
              console.log(
                utils.checkAuditTime_before30min(
                  detectionTypeArr[i].working_time
                )
              );
  ...
  getFixedChannel: function(x) {
    ...
    _my.getLocation({
          ...
          if (res) {
            if (res.success) {
              ...
              for (var i = 0; i < workingTimeArr.length; i++) {
                ...
                workingTimeArr[i] = workingTimeArr[i].substring(
                  0,
                  workingTimeArr[i].length - 3
                );

                if (
                  workingTimeArr[i].substr(
                    workingTimeArr[i].indexOf("：") + 1,
                    2
                  ) >
                  workingTimeArr[i].substr(
                    workingTimeArr[i].indexOf("-") + 1,
                  ...
                  workingTimeArr[i] = workingTimeArr[i].replace(
                    workingTimeArr[i].substr(
                      workingTimeArr[i].indexOf("-") + 1,
                      2
                    ),
              ...
              that.setData({
                appo_date:
                  that.data.objectMultiArray[that.data.multiIndex[0]].date,
                appo_time:
      ...
      fail(res) {
        console.log("获取坐标失败"); // box.showToast("获取坐标失败"+"id="+id)
      ...
      success(res) {
        ...
        request.request_get("/alipay/getFixedSamplingPoint.hn", data, function(res) {
          ...
          if (res) {
            if (res.success) {
              console.log(res.msg);
              var channelList = res.msg; // channelList.sort(function (a, b) {
  ...
  bindChooseCustodian: function(e) {
    ...
    if (that.data.account == "") {
      ...
      _my.navigateTo({
        url:
          "/pages/chooseCustodian/chooseCustodian?account=" + this.data.account
      });
    }
  },
  // //仪器类型change事件
  ...
  submitBuffer: utils.throttle(function(e) {
    ...
    if (that.data.urgent == 1) {
      if (that.data.payment_working_time != "") {
        if (!utils.checkAuditTime(that.data.payment_working_time)) {
          //当前时间是否能做指定检测类型
          that.setData({
            timeModalFlag: true
          });
  ...
  submit: utils.throttle(function(e) {
    ...
    //   appointment_date =
    //     objectMultiArray[multiIndex[0]].date +
    //     " " +
    //     objectMultiArray[multiIndex[0]].time[multiIndex[1]].time_section;
    // }
    let appointment_date = that.data.appo_date + " " + that.data.appo_time;
    //----------------------------------------------------------
    ...
    if (choose_type == 0) {
      ...
      //         res.timeStamp;
      //       let stringSignTemp =
      //         stringA + "&key=CRNmzBCrbxGVZUoz3xtmvrGBIM5YH5hJ";
  ...
  onShow: function() {
    ...
    // console.log(currPage.data.coupon_payment);
    // console.log(currPage.data.coupon_id);
    // console.log('-----------------');
    console.log(that.data.payment_amount);
  ...
  //   const key = "RLWBZ-JD53U-OJHV6-27XHB-I5VKQ-I7F3H"; //调用插件的app的名称（必填）
  ...
  getCode: function() {
    ...
    if (that.data.codeBtState) {
      ...
      if (phone == "") {
        ...
        box.showToast("验证码已发送"); //倒计时,不管验证码发送成功与否，都进入倒计时，防止多次点击造成验证码发送失败**************************
  ...
  changeTestType: function(e) {
    ...
    if (that.data.intoByScanFlag == true) {
      if (
        !utils.checkAuditTime_before30min(e.currentTarget.dataset.workingtime)
      ) {
        //当前时间不能做指定检测类型
  ...
  showMap() {
    ...
    const key = "LESBZ-6WP66-VWTSY-MBHS6-VOTVH-KBBAW"; //调用插件的app的名称（必填）
  ...
  changeOnlineFlag: function(e) {
    ...
    if (that.data.onlineid == 1) {
      //线上绑定身份
      that.setData({
        onlineFlag: true
      });
    } else if (that.data.onlineid == 2) {
      //线下刷身份证
      if (that.data.showIDcardModal == false) {
        _my.showModal({
          title: "请携带二代身份证",
          content:
            "线下刷身份证需到采样点提供证件给工作人员，若未携带可线上完成身份信息绑定",
          showCancel: false,
          confirmText: "我知道了",
          confirmColor: "#E06596",
  ...
  bindOCR: function() {
    ...
    _my.chooseImage({
      ...
      success(res) {
        ...
        const tempFilePaths = res.tempFilePaths[0]; //上传图片
        ...
        _my.uploadFile({
          ...
          //额外的参数formData
          formData: {
            api_key: "gQfz-MnGIuRUbtzJK8wB_x1ZVEc45J0H",
            //请填写你创建的 apikey
  ...
  bindCouponCheck: function() {
    ...
    _my.navigateTo({
      url:
        "/pages/coupon/couponCheck?coupon_id=" +
        that.data.coupon_id +
        "&coupon_payment=" +
        that.data.coupon_payment
    });
  },
  ...
  bindMultiPickerChange: function(e) {
    ...
    if (
      this.data.objectMultiArray[e.detail.value[0]].time[e.detail.value[1]]
        .can_use == 0
    ) {
      //提醒当前时间段不可预约
      _my.showModal({
  ...
  getbaseData: function(channel) {
    let that = this; //获取当前页面基础数据
    //获取收费规则图片，以及是否显示标志
    //获取预约须知图片，以及是否显示标志
    //获取用户服务协议
    //获取隐私政策
    console.log("getbaseData start", channel);
    let data = {
  ...
  getAppointmentTime: function() {
    //获取list
    let that = this;
    let or_data = that.data.objectMultiArray;

    let list = [];
    for (let index = 0; index < or_data.length; index++) {
      const element = or_data[index];
      let child = new Object();
      let childlist = [];
      child.name = element.date;
    ...
    my.multiLevelSelect({
      ...
      success: res => {
        ...
        for (let j = 0; j < or_data.length; j++) {
          const element = or_data[j];
          if (element.date == date) {
            let time1 = element.time;
            for (let z = 0; z < time1.length; z++) {
              const time2 = time1[z];
