const _Component = require("../__antmove/component/componentClass.js")("Component"); // components/pickerYMDHM/pickerYMDHM.js
...
_Component({
  ...
  methods: {
    _onInit() {
      ...
      let pickerArray = this.data.pickerArray; // console.log(date.getFullYear());
      //默认选择3年内
    ...
    _getModify(newDate, startDate, stopDate) {
      ...
      data.index[0] = nYear - tYear; //判断年份是否相同 相同则继续
            ...
            if (nMonth == pMonth) {
              ...
              if (nDay == tDay) {
                ...
                if (nDay == pDay) {
                  ...
                  data.index[3] = nHours; // console.log(time);
    ...
    pickerChange: function (e) {
      ...
      date.dateString = date.year + "-" + date.month + "-" + date.day + " " + date.time + ":" + date.division; // console.log(date);
    ...
    pickerColumnChange: function (e) {
      ...
      let cDivision = data.pickerArray[4][data.pickerIndex[4]].id; //需要先判断修改后的日期是否是正确的天数 不正确会导致日期错乱等未知情况

      let daysn = this._getNumOfDays(cYear, cMonth); //不正确 重新赋值
      ...
      let newDate = this._getDefaultDate(cYear + "-" + cMonth + "-" + cDay + " " + cTime + ":" + cDivision); //判断修改后的日期是否在限制范围内 不在则重新赋值
